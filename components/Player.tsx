...
const PLAYER_THEMES: Record<
  LevelType,
  { shell: string; core: string; aura: string; emissive: string; sparkle: string }
> = {
  PROLOGUE: {
    shell: '#ffebef',
    core: '#ffffff',
  ...
  NAME: {
    ...
    emissive: '#ffffff',
    sparkle: '#ffd700',
  }, // High contrast: White/Gold start
  CHEWING: {
    shell: '#98fb98',
    core: '#006400',
    aura: '#2e8b57',
  ...
  useFrame((state, delta) => {
    ...
    if (currentLevel === 'NAME') {
      ...
      if (coreRef.current) {
        (coreRef.current.material as THREE.MeshBasicMaterial).color.lerpColors(
          ...
          squeezeIntensity.current
        );
      }
    } else {
      dynamicColor.current.set(theme.shell);
      if (coreRef.current)
        (coreRef.current.material as THREE.MeshBasicMaterial).color.set(theme.core);
    }
    ...
    if (shellMaterialRef.current) {
      ...
      if (currentLevel === 'WIND') {
        if (isLevelComplete) {
          ...
          targetTransmission = THREE.MathUtils.lerp(
          ...
          targetRoughness = THREE.MathUtils.lerp(
            shellMaterialRef.current.roughness,
            0.4,
            delta * 2
          );
          ...
          if (blockFlashIntensity.current <= 0) {
            shellMaterialRef.current.emissive.lerpColors(
              new THREE.Color(theme.emissive),
    ...
    if (currentLevel === 'CHEWING') {
      ...
      envFeatures.forEach((f) => {
        if (
          f.type === 'FLESH_BALL' &&
          new THREE.Vector3(...f.position).distanceTo(pos) < 1.5 + playerScale * 0.1
        ) {
          collided = true;
        }
      });
    ...
    if (currentLevel === 'CONNECTION') {
      const { nodes, tetheredNodeId, setTetheredNode, completeConnection } =
      ...
      for (const node of nodes) {
        ...
        if (Math.sqrt(dx * dx + dz * dz) < 2.0) {
          if (!tetheredNodeId) {
            setTetheredNode(node.id);
            playConnect();
          } else if (tetheredNodeId !== node.id) {
    ...
    currentRenderScale.current = THREE.MathUtils.lerp(
      currentRenderScale.current,
      targetScale,
      delta * 2
    );
  ...
  return (
    <group ref={groupRef}>
      ...
      {interactionMode === 'SLINGSHOT' && slingshotVector && (
        <Line
          points={[new THREE.Vector3(0, 0, 0), slingshotVector.clone().multiplyScalar(-1)]}
          color="#fff"
          lineWidth={4}
          transparent
          opacity={0.5}
