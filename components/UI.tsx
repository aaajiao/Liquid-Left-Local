...
const NARRATIVE_SCRIPT: Record<LevelType, string[]> = {
  PROLOGUE: [
    '这一次,我好像是毛绒绒的一滴物。',
    '按住左键向后拉，瞄准，松开 (Drag Back & Release).',
    '向着远处的强光前进 (Head towards the Light).',
  ],
  CHAPTER_1: [
    '湿季发生了，我和其他液体从这个星球的表面上冒出来。',
    '按住左键，花纹之中总是蕴含着密码吧 (Hold LMB to Lure).',
    '拖拽丝线，连接那些神经节点 (Drag to Connect).',
  ],
  ...
  WIND: [
    '因为液体干枯的时候，会往我的身体里缩，好痛好痛...',
    '我能感觉自己在慢慢地变小，慢慢地变暗...',
    '没关系，我在抗拒生命的时候已经感受到了更强的生命力...',
  ],
  ...
  HOME: [
    '滴滴, 这就是我们身体性的语言, 我们的家园。',
    '原来它掉进了湖泊里, 而湖就是由我们液体的眼泪组成的呀。',
  ],
  SUN: [
    '我不害怕。我不在了，湿也永远会在。',
    '而这不是一场降临，只是无限循环中的一部分呀。',
    '点击蘑菇，降下甘霖。',
  ],
};
...
export const UI: React.FC = () => {
  ...
  let text =
    NARRATIVE_SCRIPT[currentLevel][
      Math.min(narrativeIndex, NARRATIVE_SCRIPT[currentLevel].length - 1)
    ];
        ...
        <div className="w-full max-w-3xl px-2 md:px-4 text-center pointer-events-auto mx-auto mb-12 md:mb-16">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <p className="text-lg md:text-xl lg:text-2xl text-slate-800/80 font-medium leading-relaxed drop-shadow-sm h-16">
                {text}
              </p>
              {isLevelComplete && currentLevel !== 'SUN' && (
                <motion.button
                  ...
                  className="mt-6 md:mt-8 text-pink-500 border border-pink-300 px-4 py-1.5 md:px-6 md:py-2 text-sm md:text-base rounded-full hover:bg-pink-50 transition-colors"
                >
                  前往下一章 (Proceed)
                </motion.button>
              )}
              {currentLevel === 'SUN' && isLevelComplete && (
                <motion.button
                  ...
                  className="mt-6 md:mt-8 text-blue-400 hover:text-blue-600 text-sm md:text-base"
                >
                  [ 轮回 (Rebirth) ]
                </motion.button>
              )}
