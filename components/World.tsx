...
const PhysicsPlane: React.FC = () => {
  ...
  return (
    <mesh
      ...
      visible={true}
      onPointerMove={(e) => {
        if (e.isPrimary) setCursorWorldPos(e.point);
      }}
      onPointerDown={(e) => {
        if (e.isPrimary && e.button === 0) setMouseDown(true);
      }}
      onPointerUp={(e) => {
        if (e.isPrimary) {
          setMouseDown(false);
  ...
  return (
    <group>
      ...
      {currentLevel === 'HOME' ? (
        ...
        <BreathingGround level={currentLevel} rainLevel={rainLevel} />
      )}
...
const BreathingGround: React.FC<{ level: string; rainLevel?: number }> = ({
  level,
  rainLevel = 0,
}) => {
...
const WindDanmakuSystem: React.FC = () => {
  ...
  useFrame((state, delta) => {
    ...
    if (!isLevelComplete && Math.random() < 0.1) {
      // Spawn chance
      for (let i = 0; i < count; i++) {
        if (bullets.current[i * STRIDE + 5] === 0) {
          // Find inactive
...
const RealisticMushroom: React.FC<{ feature: EnvFeature }> = ({ feature }) => {
  ...
  return (
    <group
      position={[feature.position[0], feature.position[1] + yOffset, feature.position[2]]}
      onClick={handleClick}
      onPointerOver={handleOver}
      onPointerOut={handleOut}
      scale={scale}
...
const SunFeature: React.FC<{ feature: EnvFeature; rainLevel?: number }> = ({
  feature,
  rainLevel = 0,
}) => {
  ...
  return (
    <group
      position={feature.position}
      scale={[feature.scale[0] * scale, feature.scale[1] * scale, feature.scale[2] * scale]}
      onPointerOver={handleOver}
      onPointerOut={handleOut}
    >
      {/* Core Sun */}
...
const WitheredLeafFeature: React.FC<{ feature: EnvFeature }> = ({ feature }) => {
  ...
  return (
    <group position={feature.position} scale={feature.scale} rotation={[-Math.PI / 3, 0, 0]}>
      ...
      <Sparkles
        count={10 + Math.floor(ratio * 30)}
        scale={3}
        color={ratio > 0.5 ? '#00ff00' : '#d2691e'}
        size={ratio > 0.5 ? 6 : 3}
        speed={0.5 + ratio}
        opacity={0.6}
      />
...
const OrganicFeature: React.FC<{ feature: EnvFeature }> = ({ feature }) => {
  ...
  if (feature.type === 'FLESH_TUNNEL') {
    return (
      <mesh
        position={feature.position}
        rotation={new THREE.Euler(...(feature.rotation || [0, 0, 0]))}
        scale={feature.scale}
      >
        <sphereGeometry args={[1, 32, 32]} />
  ...
  if (feature.type === 'ORGANIC_PLATFORM') {
    return (
      <mesh position={feature.position} receiveShadow>
        <cylinderGeometry
          args={[feature.scale[0] / 2, feature.scale[0] / 2, feature.scale[1], 32]}
        />
    ...
    return (
      <group position={feature.position}>
        <mesh rotation={[-Math.PI / 2, 0, 0]}>
          <circleGeometry args={[feature.scale[0] / 2, 64]} />
          <meshBasicMaterial color="#87cefa" transparent opacity={0.8} />
        </mesh>
...
const Atmosphere: React.FC<{ level: string }> = ({ level }) => {
  ...
  return (
    <>
      ...
      {level === 'CHAPTER_1' && (
        <Sparkles count={50} scale={20} size={4} speed={0.4} opacity={0.5} color="#fff0f5" />
      )}
      {level === 'NAME' && (
        <Sparkles count={80} scale={15} size={3} speed={0.2} opacity={0.4} color="#8a2be2" />
      )}
      ...
      {level === 'SUN' && (
        <>
          ...
          {rainLevel > 0 && (
            <Cloud
              opacity={Math.min(rainLevel / 10, 0.8)}
              speed={0.5}
              bounds={[20, 4, 20]}
              color="#ffadad"
              position={[0, rainLevel / 2, 0]}
            />
          )}
        </>
      )}
