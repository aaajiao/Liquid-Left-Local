...
const generateLevel1Env = () => {
  ...
  positions.forEach((pos, i) =>
    nodes.push({ id: `n1-${i}`, position: [pos[0], 0.5, pos[2]], connected: false })
  );
  for (let i = 0; i < 8; i++)
    env.push({
      id: `spore-${i}`,
    ...
    while (!valid && attempts < 20) {
      ...
      for (const n of nodes) {
        const dist = Math.sqrt(
          Math.pow(pos[0] - n.position[0], 2) +
            Math.pow(pos[1] - n.position[1], 2) +
            Math.pow(pos[2] - n.position[2], 2)
        );
        if (dist < 4.0) {
          valid = false;
          break;
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  completeConnection: (targetId) => {
    ...
    const exists = connections.some(
      (c) => (c[0] === sourceId && c[1] === targetId) || (c[0] === targetId && c[1] === sourceId)
    );
    ...
    if (!exists) {
      if (currentLevel === 'CHAPTER_1') {
        ...
        const isValid =
          (sourceId === currentSource && targetId === currentTarget) ||
          (sourceId === currentTarget && targetId === currentSource);
        if (!isValid) {
          set({ draggingNodeId: null });
          return;
        }
      ...
      const newNodes = nodes.map((n) =>
        n.id === sourceId || n.id === targetId ? { ...n, connected: true } : n
      );
  ...
  startLevel: (level) => {
    ...
    switch (level) {
      case 'PROLOGUE':
        genResult = generatePrologueEnv();
        mode = 'SLINGSHOT';
        break;
      case 'CHAPTER_1':
        genResult = generateLevel1Env();
        seq = genResult.nodes.map((n) => n.id);
        break;
      case 'NAME':
        genResult = generateNameEnv();
        break;
      case 'CHEWING':
        genResult = generateChewingEnv();
        break;
      case 'WIND':
        genResult = generateWindEnv();
        break;
      case 'TRAVEL':
        genResult = generateTravelEnv();
        break;
      case 'CONNECTION':
        genResult = generateConnectionEnv();
        break;
      case 'HOME':
        genResult = generateHomeEnv();
        mode = 'OBSERVER';
        break;
      case 'SUN':
        genResult = generateSunEnv();
        mode = 'CLICK';
        break;
    }
