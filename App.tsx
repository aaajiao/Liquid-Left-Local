...
const CustomCursor: React.FC = () => {
  ...
  return (
    ...
    >
      {/* Core Light */}
      <div
        className={`absolute -translate-x-1/2 -translate-y-1/2 rounded-full bg-white transition-all duration-200 shadow-[0_0_10px_#fff]
      ...
      {/* Outer Glow / Halo */}
      <div
        className={`absolute -translate-x-1/2 -translate-y-1/2 rounded-full transition-all duration-300 blur-sm
      ...
      {/* Interaction Ring */}
      <div
        className={`absolute -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/60 transition-all duration-300
            ${
              hoveredNodeId || isInteractiveHover
                ? 'w-12 h-12 scale-100 rotate-180 opacity-100 border-2 bg-white/10'
                : 'w-4 h-4 scale-0 opacity-0'
            }
        `}
...
const DynamicBackground: React.FC = () => {
  ...
  useFrame((state, delta) => {
    ...
    if (!scene.fog) scene.fog = new THREE.Fog(fogColorRef.current, 10, 60);
      ...
      (scene.fog as THREE.Fog).far = currentLevel === 'HOME' ? 80 : 40;
    }
  });
  return null;
};
